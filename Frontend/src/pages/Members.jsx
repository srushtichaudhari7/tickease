...
const Members = () => {
    ...
    const handleMemberSubmit = async () => {
        ...
        try {
            const response = await axiosInstance.post("http://localhost:4000/api/members", newMember, {
                headers: { Authorization: `Bearer ${localStorage.getItem("token")}` },
            });
...
{isModalOpen && (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center">
        <div className="bg-white p-6 rounded-lg shadow-lg w-96">
            <h2 className="text-xl font-bold mb-4 text-blue-600">Add New Member</h2>
            <input
                type="text"
            ...
            <div className="flex justify-end">
                ...
                <button className="bg-blue-500 text-white px-4 py-2 rounded-md" onClick={handleMemberSubmit}>Save</button>
            </div>
        </div>
    </div>
)}
